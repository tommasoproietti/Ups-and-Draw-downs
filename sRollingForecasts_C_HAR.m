...
clear all;  % conditional forecasts based on multistep absolute returns predictions
%% Graphic parameters
...
for i = 0:cr-1
    ...
    mvar_f_har = zeros(ctau+1,ctau); vdd_f_cond = NaN(cL,1);
    for t = 1:ctau
        vars = mars(:,t);
        if any(i==0:10:(cr-1))
            % HAR specification
        ...
        [var_f_har, ~] = forecast(EstMdl_har, cL,vars);
        mvar_f_har(2:end,t) = var_f_har;
...
save sRollingForecasts_C_HAR.mat
